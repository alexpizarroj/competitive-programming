...
void fill_bucket_data(vector<Elephant>& v) {
  const int N = v.size();
  int next = N;
...
void build_buckets(const vector<pair<int, int> >& X) {
  int covered = 0;
...
void init(int N, int L, int X[]) {
  ...
  n_buckets = 1;
  while (n_buckets * n_buckets < N) n_buckets += 1;
  buckets_span = n_buckets;
  update_cnt = 0;
  epos.resize(N);

  vector<pair<int, int> > XX(N);
  for (int i = 0; i < N; i++) XX[i] = make_pair(X[i], i);
  build_buckets(XX);
}
